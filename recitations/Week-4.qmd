...
Suppose we wanted to test if the wage gap is large for married vs unmarried couples. The estimand (i.e. the population parameter we are going for) is:

$$\left(\mathbb{E}[\log(W)|\text{Female,Married}] - \mathbb{E}[\log(W)|\text{Male,Married}]\right) - \left(\mathbb{E}[\log(W)|\text{Female,Single}] - \mathbb{E}[\log(W)|\text{Male,Single}]\right) $$
...
### Exercise:
Show that the population parameter above is equal to $\beta_{3}$.
