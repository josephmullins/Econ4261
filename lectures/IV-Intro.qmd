...
## Example 3: Elasticity of Demand {.smaller}
...
## Example 1: Use the distance to nearest college {.smaller}

**Exercise**:
$$ \alpha = \frac{\mathbb{C}(Y,Z)}{\mathbb{C}(D,Z)}  $$
...
**Exercise**:
$$ \alpha = \frac{\mathbb{C}(Y,Z)}{\mathbb{C}(D,Z)} = \frac{\mathbb{E}[Y|Z=1] - \mathbb{E}[Y|Z=0]}{\mathbb{E}[D|Z=1] - \mathbb{E}[D|Z=0]} $$
...
d2 <- d %>%
    group_by(Z) %>%
    mutate(p = mean(p),q = mean(q)) %>%
    mutate(stage = "Average prices and quantities by Z") 

d4 <- d2 %>%
    mutate(alpha_line = 1,stage = "Elasticity is inverse of slope") 


ggplot(rbind(d,d2,d4),aes(q,p,color=Z)) + geom_point(size=2) + theme_minimal() + transition_states(stage) + geom_line(aes(x=q,y=p,alpha = as.factor(alpha_line)),color="black",size=1.5) + scale_alpha_discrete(range = c(0, 1),guide="none") +  labs(title = '{closest_state}')
