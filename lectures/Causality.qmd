...
## How to infer causality {.smaller}

Here's the recipe:

::: {.incremental}

1. Start with a **model of the world**: $Y = h_{\theta}(X,W,\eta)$. This is your theory of how the data is generated. $\eta$ is what we call **structural unobservable**. The model $h$ is indexed by population parameters, $\theta$. 
...
3. State the assumptions under which your counterfactual can be constructed 
...
We can exploit the **randomness** conditional on $b$ to infer causality.
...
- Suppose that the variable $D$ could be *assigned randomly* $\Rightarrow$ $\eta_{0},\eta_{1} \perp D$.
